function [Q, ets, numberOfStaps4Episode, actions10e3] = q_learning_algorithm(alpha, rowsNumber, columnsNumber, inicialState, terminalState, wind, MAX_EPISODE)
...
%V contains the value os the utility of an space resulting of the some
%action, captured of Q
    ...
    for episode = 1:MAX_EPISODE
        ...
        while(~(actualState(1) == terminalState(1) && actualState(2) == terminalState(2)))
            ...
            if(~(nextState(1) == terminalState(1) && nextState(2) == terminalState(2))) %if it is not the terminal state
                [V(nextState(1),nextState(2)),aux] = max(Q(ns,nextAction));
                Q(as,action) = Q(as,action) + alpha*( reward + gamma * V(nextState(1),nextState(2)) - Q(as,action));
